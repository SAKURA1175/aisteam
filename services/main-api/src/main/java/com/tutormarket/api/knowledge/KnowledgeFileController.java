...
public class KnowledgeFileController {
    ...
    public KnowledgeDtos.KnowledgeFileResponse upload(@PathVariable UUID teacherId,
        ...
        return knowledgeFileService.uploadTeacherPublic(
                teacherId,
                file,
                actor,
    ...
    public List<KnowledgeDtos.KnowledgeFileResponse> list(@PathVariable UUID teacherId,
                                                          @AuthenticationPrincipal UserPrincipal actor) {
        return knowledgeFileService.listTeacherPublic(teacherId, actor);
    }

    @GetMapping("/knowledge-files/{fileId}")
    public KnowledgeDtos.KnowledgeFileResponse get(@PathVariable UUID fileId,
                                                   @AuthenticationPrincipal UserPrincipal actor) {
        return knowledgeFileService.getTeacherPublicById(fileId, actor);
    }
    ...
    public void delete(@PathVariable UUID fileId,
                       @AuthenticationPrincipal UserPrincipal actor,
                       HttpServletRequest request) {
        knowledgeFileService.deleteTeacherPublic(fileId, actor, (String) request.getAttribute(RequestIdFilter.REQUEST_ID_ATTRIBUTE));
    }

    @PostMapping("/knowledge-files/{fileId}/retry")
    public KnowledgeDtos.KnowledgeFileResponse retry(@PathVariable UUID fileId,
                                                     @AuthenticationPrincipal UserPrincipal actor,
                                                     HttpServletRequest request) {
        return knowledgeFileService.retryTeacherPublic(
                fileId,
                actor,
                (String) request.getAttribute(RequestIdFilter.REQUEST_ID_ATTRIBUTE)
        );
